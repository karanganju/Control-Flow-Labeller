...
int main(int argc, char *argv[]) {
	...
	for (map<string, list<pair<string, int> > >::iterator i = callee_graph.begin(); i != callee_graph.end(); i++) {
		list<int> temp;
		for (list<pair<string, int> >::iterator j = i->second.begin(); j != i->second.end(); j++) callee_graph_stripped[str_to_int[i->first]].push_back(str_to_int[j->first]);
	}
	...
	// cout << SCC_count << " " << count << endl;
	// for (int i = 0; i < SCC_count; i++) {
	// 	for (int j = 0; j < SCC_count; j++) cout << SCC_transitions[i * SCC_count + j] << " ";
